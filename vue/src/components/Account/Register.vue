<template>
    <account-index title="邮箱注册" rightTitle="短信注册" @rightTitleClick="rightTitleClick">
        <template #form>
            ...
            >
                <template #button>
                    <span @click="getVCode" :class="enableEmail?'rem-accent':'rem-disabled'">获取验证码</span>
                </template>
            </van-field>
            <van-field
                    v-model="vcode"
            ...
            <div class="rem-form-buttons">
                <v-btn :to="{name:'Login'}" class="rem-btn" depressed large outlined color="secondary">登陆</v-btn>
                <v-btn
                        :class="enableRegister?'rem-btn':'rem-btn rem-btn-disable'"
                        depressed large color="secondary"
                        :disabled="!enableRegister"
                        @click="register">同意并注册
                </v-btn>
            </div>
        </template>
        <template #explain>
            完成注册即代表你同意<a target="_blank" href="#">用户协议</a>和<a target="_blank" href="#">隐私政策</a>
        </template>
        <template #footer>
            <van-popup
                    v-model="showLayer"
    ...
    export default {
        ...
        computed: {
            enableRegister: function () {
                return this.email.trim() !== "" && this.vcode !== "" && this.password !== "" && this.confirmPwd !== "";
            },
